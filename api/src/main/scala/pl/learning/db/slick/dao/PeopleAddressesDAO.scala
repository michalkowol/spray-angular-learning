...
import pl.learning.db.{Person, Address}
...
object PeopleAddressesDAO {
  def createRelation(person: Person, address: Address)(implicit ec: ExecutionContext): Future[Int] = {
    val insert = addressesPeople.map(ap => (ap.personId, ap.addressId)).returning(addressesPeople) += (person.id.get, address.id.get)
    db.run(insert).map(_.id.get)
  }
}

...
object GatheringActor2 {
  def props(serviceA: ActorRef, serviceB: ActorRef, serviceC: ActorRef) = {
    Props(new GatheringActor2(serviceA, serviceB, serviceC))
  }
}

class GatheringActor2(serviceA: ActorRef, serviceB: ActorRef, serviceC: ActorRef) extends Actor with ActorLogging {
  ...
  def collectResults() = (responseA, responseB, responseC) match {
    case (Some(a), Some(b), Some(c)) =>
      context.parent ! ResponseABC(a, b, c)
      self ! PoisonPill
    case _ =>
  }
